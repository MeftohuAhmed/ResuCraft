...
/**
 * Component for managing employee education information.
 * It allows users to add, edit, and remove education entries.
 */
const EmployeeEducation = () => {
    // Selector to get the 'education' state from Redux store
    const { education } = useSelector((store) => store);
    const dispatch = useDispatch();

    // Local state for the form fields related to education
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            // Dispatch action to save education when the form is submitted
            if (formik.isValid) {
        ...
        <>
            <React.Fragment>
                <FormikProvider value={formik}>
                    {/* FieldArray for managing dynamic education fields */}
                    <FieldArray
                        name="education"
                        render={(arrayHelpers) => (
                            <AddCircleIcon
                    ...
                    <FieldArray
                        ...
                        render={(arrayHelpers) => (
                            <div>
                                {/* Map through education entries and render form fields */}
                                {formik.values.education.map((education, index) => {
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            {index > 0 && (
                                                // CloseIcon for removing education entry
                                                <CloseIcon
                                                    sx={{
                                                        float: "right",
                                                        fontSize: "30px",
                                            ...
                                            <form className="form-group">
                                                <Grid container spacing={4}>
                                                    <Grid item xs={6} className="item">
                                                        {/* InputField component for Academic Institute */}
                                                        <InputField
                                                    ...
                                                    <Grid item xs={6} className="item">
                                                        {/* InputField component for Degree */}
                                                        <InputField
                                                            label="Degree"
                                                            type="text"
                                                            placeholder="e.g. BS"
                                                    ...
                                                    <Grid item xs={6} className="item">
                                                        {/* InputField component for Field of Study */}
                                                        <InputField
