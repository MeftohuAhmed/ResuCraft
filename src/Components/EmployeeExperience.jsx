...
/**
 * Component for managing employee work experience information.
 * It allows users to add, edit, and remove work experience entries.
 */
const EmployeeExperience = () => {
    // Selector to get the 'work' state from Redux store
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            // Dispatch action to save work experience when the form is submitted
            if (formik.isValid) {
    ...
    return (
        <React.Fragment>
            <FormikProvider value={formik}>
                {/* FieldArray for managing dynamic work experience fields */}
                <FieldArray
                    name="work"
                    render={(arrayHelpers) => (
                        <AddCircleIcon
                            ...
                            {formik.values.work.map((experience, index) => {
                                return (
                                    <React.Fragment key={index}>
                                        {index > 0 && (
                                            // CloseIcon for removing work experience entry
                                            <CloseIcon
                                                sx={{
                                                    float: "right",
                                                    fontSize: "30px",
                                        ...
                                        <form className="form-group">
                                            {/* ExperienceForm component to handle individual experience entry */}
                                            <ExperienceForm
